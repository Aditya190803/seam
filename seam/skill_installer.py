...
def install_agent_skill(*, codex: bool = True, claude: bool = True, project: bool = False, root: Path | None = None) -> list[SkillInstallResult]:
    ...
    if codex:
        targets.append(("Codex", base / ".agents" / "skills" / SKILL_NAME if project else base / ".agents" / "skills" / SKILL_NAME))
    if claude:
        targets.append(("Claude Code", base / ".claude" / "skills" / SKILL_NAME if project else base / ".claude" / "skills" / SKILL_NAME))
